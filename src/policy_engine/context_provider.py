...
Assembles the scalar values that policy conditions can reference:
  ...
  cloud_cover_pct int     Total cloud cover 0-100 %.
                          Defaults to 0 (clear) on failure so a missing
                          reading never triggers a cloud-based policy.

  is_overcast     bool    True when cloud_cover_pct > 70.
...
_DEFAULT_CLOUD_COVER    = 0      # clear sky — avoids false overcast triggers
_DEFAULT_IS_OVERCAST    = False  # clear — avoids false overcast triggers
...
def get_context() -> Dict[str, Any]:
    ...
    if cloud_cover is None:
        cloud_cover = _DEFAULT_CLOUD_COVER

    is_overcast = weather.get("is_overcast", _DEFAULT_IS_OVERCAST)

    return {
        "temperature":     round(temp_f, 1),
        "humidity":        float(humidity),
        "cloud_cover_pct": int(cloud_cover),
        "is_overcast":     bool(is_overcast),
